            ...
            <div class="multi-steps">
                <div class="col-md-12 col-xs-12 col-sm-12 left-side">
                    <div class="steps" id="step1">
                        ...
                        <form method="post" action="/user/content/venu_update/{{ $item->id }}" class="form-horizontal" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-2 tab-con">
                                    <input type="text" name="venu_size" placeholder="Enter your course duration" class="form-control" value="{{ $item->venu_size }}">
                                </div>
                                
                                <label class="control-label col-md-2 tab-con" for="inputDefault">Available Seat*</label>
                                <div class="col-md-2 tab-con">
                                    <input type="text" name="available_seat" placeholder="Enter availble seat" class="form-control" value="{{ $item->available_seat }}">
                                </div>
                                ...
                                <div class="col-md-2 tab-con">
                                    <select name="availability_type" class="form-control font-s availibility_type">
                                        ...
                                        <option value="hour" @if($item->availability_type == "hour") selected="selected" @endif> Hour</option>
                                        <option value="day" @if($item->availability_type == "day") selected="selected" @endif> Day</option>
                                    </select>
                                </div>
                                <div class="col-md-2 tab-con price_box">
                                    <input type="text" name="availability_hour" placeholder="Enter availibility hour" class="form-control" value="{{ $item->availability_hour }}">
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-2 tab-con">
                                    <input type="text" name="regular_price" placeholder="Enter previous price" class="form-control" value="{{ $item->price }}">
                                </div>
                                <div class="col-md-2 tab-con">
                                    <input type="radio" name="price_type" value='hour' @if($item->price_type == "hour") checked @endif> Hour
                                    <input type="radio" name="price_type" value='day' @if($item->price_type == "day") checked @endif> Day
                                </div>
                                
                                <label class="control-label col-md-2 tab-con" for="inputDefault">Discount Price(Tk)</label>
                                <div class="col-md-2 tab-con">
                                    <input type="text" name="discount_price" placeholder="Enter discount price" class="form-control" value="{{ $item->discount_price }}">
                                </div>
                                <div class="col-md-2 tab-con discount_price_type">
                                    <input type="radio" name="discount_price_type" value='hour' @if($item->discount_price_type == "hour") checked @endif> Hour
                                    <input type="radio" name="discount_price_type" value='day' @if($item->discount_price_type == "hour") checked @endif> Day
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                @if(count($free_features) > 0)
                                    @foreach($free_features as $list)
                                        <input type="checkbox" name="free_feature[]" value="{{ $list->name }}" @if(in_array($list->name, json_decode($item->free_extra_feature))) checked @endif> {{ $list->name }}
                                    @endforeach
                                @endif
                                ...
                                <div class="col-md-10 tab-con">
                                 @if(count($paid_features) > 0)
                                    @foreach($paid_features as $list)
                                        <input type="checkbox" name="paid_feature[]" value="{{ $list->name }}" @if(in_array($list->name, json_decode($item->paid_extra_feature))) checked @endif> {{ $list->name }}
                                    @endforeach
                                @endif
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-10">
                                    ...
                                    <div>
                                        @if($item->image)
                                        <img src="/uploads/venu_thumbnails/{{ $item->image }}" id="profilePhotoViewer1" width="300" style="height:150px" class="img img-responsive img-thumbnail" style="border-radius: 10px;">
                                        @else
