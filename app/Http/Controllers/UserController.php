...
use App\Models\CourseFacility;
use App\Models\UserSocial;
use App\Models\VenuFeature;
...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $total_course = Content::where('type', '=', 'course')->where('user_id', $user->id)->count();
        $total_venu   = Content::where('type', '=', 'venu')->where('user_id', $user->id)->count();
    ...
    public function userProfile()
    {
        ...
        return view(getTemplate() . '.user.pages.profile', ['user' => $user, 'meta' => $userMetas, 'divisions' => $divisions]);
    }
    ...
    {
        $user = auth()->user();
        $lists = UserSocial::where('user_id', $user['id'])->where('is_show', 1)->get();
        return view(getTemplate() . '.user.social.list', ['lists' => $lists]);
    }
    ...
    {
        $user = auth()->user();
        UserSocial::where('id', $id)->where('user_id', $user->id)->delete();
    ...
    public function instructorList()
    {
        ...
        $user  = auth()->user();
        $lists = UserInstructor::where('user_id', $user['id'])->where('is_show', 1)->get();
        $instructors = InstructorInfo::where('is_active', 1)->get();
        
        return view(getTemplate() . '.user.instructor.list', ['lists' => $lists, 'instructors' => $instructors]);
    }
     ...
     public function instructorStore(Request $request){
        ...
        $instructor->instructor_id     = $request->input('name');
        $instructor->is_show  = $request->input('status');
    ...
    {
        $user = auth()->user();
        UserInstructor::where('id', $id)->where('user_id', $user->id)->delete();
        return redirect('/user/instructor');
    }
    ...
    {
        // $user = auth()->user();
        // $userContentsQuery = Content::where('user_id', $user->id)->where('mode', 'publish');
        // $videos = $userContentsQuery->with('metas')->paginate(10);
 
        // $user = auth()->user();
        // $lists = Content::where('user_id', $user->id)->with('category')->withCount('sells', 'partsactive')->orderBy('id', 'DESC')->paginate(10);
        // return view(getTemplate() . '.user.gallery.list', ['lists' => $lists]);
        
        $user = auth()->user();
        $galleries = Gallery::where('user_id', $user->id)->get();

        return view(getTemplate() . '.user.gallery.list', ['galleries' => $galleries]);
    }
    ...
    public function galleryStore(Request $request){
        ...
        if ($request->file('thumbnail_photo')) {
            ...
            if(!in_array($mime_type,['image/jpeg','image/jpg','image/png'])){
                return redirect('user/gallery')->with('flash_danger','Gallery image must be png or jpg or jpeg format!');
            }
    ...
    {
        $user = auth()->user();
        Gallery::where('id', $id)->where('user_id', $user->id)->delete();
    ...
    public function venuList()
    {
        $user = auth()->user();
        // $lists = Content::where('user_id', $user->id)->with('category')->withCount('sells', 'partsactive')->orderBy('id', 'DESC')->paginate(10);
        $lists = Content::where('user_id', $user->id)->where('type', '=', 'venu')->orderBy('id', 'DESC')->paginate(10);
        return view(getTemplate() . '.user.venu.list', ['lists' => $lists]);
    }
    ...
    {
        $user = auth()->user();
        // $lists = Content::where('user_id', $user->id)->where('type', 'course')->with('category')->withCount('sells', 'partsactive')->orderBy('id', 'DESC')->paginate(10);
        $lists = Content::where('user_id', $user->id)->where('type', '=', 'course')->orderBy('id', 'DESC')->paginate(10);
     ...
     public function createCourse()
    {
        ...
        $facilities = CourseFacility::where('is_show', 1)->get();
        $instructors = InstructorInfo::where('is_active', 1)->get();
        ...
        return view(getTemplate() . '.user.content.course_create', ['instructors' => $instructors, 'facilities' => $facilities, 'user' => $user, 'menus' => $contentMenu, 'lists' => $list, 'divisions' => $divisions, 'districts' => $districts, 'upazilas' => $upazilas ]);
    }
    
    public function storeCourse(Request $request)
    {
        $sliders    = $request->file('slider_photos');
        ...
        // $instructor_info->name  = $request->input('instructor_name');

        // $ins_prefix = date('Ymd_');
        // $ins_photo  = $request->file('instructor_image');

        // if ($request->file('instructor_image')) {
        //     $mime_type = $ins_photo->getClientMimeType();
        //     if(!in_array($mime_type,['image/jpeg','image/jpg','image/png'])){
        //         return redirect('user/content/course/create')->with('flash_danger','Instructor image must be png or jpg or jpeg format!');
        //     }
        //     $photoFile = trim(sprintf("%s", uniqid($ins_prefix, true))) .'.'.$ins_photo->getClientOriginalExtension();
        //     if (!file_exists('uploads/instructor_images/')) {
        //         mkdir('uploads/instructor_images', 0777, true);
        //     }
        //     $ins_photo->move('uploads/instructor_images/', $photoFile);
        //     $instructor_info->image = $photoFile;
        // }
        ...
        $newContent->content              = $request->input('content_details');
        $newContent->category_id          = $request->input('course_category');
        $newContent->instructor_id        = $request->input('instructor_id');
        $newContent->content_type         = $request->input('course_type');
        $newContent->division             = $request->input('division');
        $newContent->district             = $request->input('district');
        $newContent->upazila              = $request->input('upazila');
        $newContent->address              = $request->input('address');
        $newContent->facilities           = json_encode($facility_list);
        $newContent->course_duration      = $request->input('course_duration');
        $newContent->course_duration_type = $request->input('course_duration_type');
        $newContent->course_start_date    = $request->input('course_start_time');
        $newContent->available_seat       = $request->input('available_seat');
        $newContent->discount_price       = $request->input('discount_price');
        $newContent->negotiable           = $request->input('negotiable');
        $newContent->free                 = $request->input('free');
        $newContent->price                = $request->input('regular_price');
        $newContent->live_url             = $request->input('live_url');
        $newContent->user_id              = $user->id;
        $newContent->type                 = 'course';
        $newContent->mode                 = 'publish';
        $newContent->contact_number       = $request->input('number');
        $newContent->created_at           = time(); // this time format need to change from table also
        ...
        if ($request->file('thumbnail_photo')) {
            $mime_type = $photo->getClientMimeType();
            if(!in_array($mime_type,['image/jpeg','image/jpg','image/png'])){
                return redirect('user/content/course/create')->with('flash_danger','Thumbnail image must be png or jpg or jpeg format!');
            }
            $photoFile = trim(sprintf("%s", uniqid($prefix, true))) .'.'.$photo->getClientOriginalExtension();
            if (!file_exists('uploads/content_thumbnails/')) {
                mkdir('uploads/content_thumbnails', 0777, true);
            }
            $photo->move('uploads/content_thumbnails/', $photoFile);
            $newContent->thumbnail_photo = $photoFile;
        }
        $newContent->save();
         ...
         if($sliders){
            foreach($sliders as $slider){
                ...
                $content_slider->content_id = $newContent->id;
                $slider_prefix = date('Ymd_');
                $slider_photo  = $slider;
        
                if ($request->file('slider_photos')) {
                    $mime_type = $slider_photo->getClientMimeType();
                    if(!in_array($mime_type,['image/jpeg','image/jpg','image/png'])){
                        return redirect('user/content/course/create')->with('flash_danger','Slider image must be png or jpg or jpeg format!');
                    }
                    $photoFile = trim(sprintf("%s", uniqid($slider_prefix, true))) .'.'.$slider_photo->getClientOriginalExtension();
                    if (!file_exists('uploads/content_sliders/')) {
                        mkdir('uploads/content_sliders', 0777, true);
                    }
                    $slider_photo->move('uploads/content_sliders/', $photoFile);
                    $content_slider->slider = $photoFile;
                }
                $content_slider->save(); 
            }
        }
        ...
        return redirect('user/dashboard')->with('flash_success','Course create successfully!');
    ...
    public function createVenu()
    {
        ...
        $paid_features = VenuFeature::where('is_show', 1)->where('payment_status', 1)->get();
        $free_features = VenuFeature::where('is_show', 1)->where('payment_status', 0)->get();
        
        $divisions = DB::table('location_divisions')->get();
        $districts = DB::table('location_districts')->get();
        $upazilas  = DB::table('location_upazillas')->get();
        
        return view(getTemplate() . '.user.content.venu_create', ['free_features' => $free_features, 'paid_features' => $paid_features, 'user' => $user, 'menus' => $contentMenu, 'divisions' => $divisions, 'districts' => $districts, 'upazilas' => $upazilas]);
    }
    
    public function storeVenu(Request $request)
    {
        $sliders           = $request->file('slider_photos');
        $free_feature      = $request->input('free_feature');
        $paid_features     = $request->input('paid_feature');
        $availability_days = $request->input('availability_day');
        $user = auth()->user();
        
        $free_feature_list = [];
        $paid_feature_list = [];
        // if($free_feature){
        //     foreach($free_feature as $facility){
        //         $free_feature_list[] = $facility;
        //     }
        // }
        // if($paid_features){
        //     foreach($paid_features as $facility){
        //         $paid_feature_list[] = $facility;
        //     }
        // }
        ...
        $newContent->title                = $request->input('title');
        $newContent->content              = $request->input('content_details');
        $newContent->category_id          = 26; // TODO:: this id must same as category venue id
        $newContent->division             = $request->input('division');
        $newContent->district             = $request->input('district');
        $newContent->upazila              = $request->input('upazila');
        $newContent->address              = $request->input('address');
        $newContent->free_extra_feature   = json_encode($free_feature);
        $newContent->paid_extra_feature   = json_encode($paid_features);
        $newContent->venu_size            = $request->input('venu_size');
        $newContent->availability_type    = $request->input('availability_type');
        $newContent->availability_hour    = $request->input('availability_hour');
        $newContent->availability_day     = isset($availability_days) ? json_encode($availability_days) : null;
        $newContent->available_seat       = $request->input('available_seat');
        $newContent->discount_price       = $request->input('discount_price');
        $newContent->discount_price_type  = $request->input('discount_price_type');
        // $newContent->discount_price_days  = isset($price_in_days) ? json_encode($price_in_days) : null;
        $newContent->price                = $request->input('regular_price');
        $newContent->price_type           = $request->input('price_type');
        $newContent->user_id              = $user->id;
        $newContent->type                 = 'venu';
        $newContent->mode                 = 'publish';
        $newContent->contact_number       = $request->input('number');
        $newContent->created_at           = time(); // this time format need to change from table also
        ...
        if ($request->file('thumbnail_photo')) {
            $mime_type = $photo->getClientMimeType();
            if(!in_array($mime_type,['image/jpeg','image/jpg','image/png'])){
                return redirect('user/content/venu/create')->with('flash_danger','Thumbnail image must be png or jpg or jpeg format!');
            }
            $photoFile = trim(sprintf("%s", uniqid($prefix, true))) .'.'.$photo->getClientOriginalExtension();
            if (!file_exists('uploads/venu_thumbnails/')) {
                mkdir('uploads/venu_thumbnails', 0777, true);
            }
            $photo->move('uploads/venu_thumbnails/', $photoFile);
            $newContent->thumbnail_photo = $photoFile;
        }
        $newContent->save();
         ...
         if($sliders){
            foreach($sliders as $slider){
                ...
                if ($request->file('slider_photos')) {
                    $mime_type = $slider_photo->getClientMimeType();
                    if(!in_array($mime_type,['image/jpeg','image/jpg','image/png'])){
                        return redirect('user/content/venu/create')->with('flash_danger','Slider image must be png or jpg or jpeg format!');
                    }
                    $photoFile = trim(sprintf("%s", uniqid($slider_prefix, true))) .'.'.$slider_photo->getClientOriginalExtension();
                    if (!file_exists('uploads/content_sliders/')) {
                        mkdir('uploads/content_sliders', 0777, true);
        ...
        // $content_id = Content::insertGetId($newContent);
        // return redirect('/user/content/edit/' . $content_id);
        return redirect('user/dashboard')->with('flash_success','Venu create successfully!');

    }
    
    public function venuEdit($id)
    {
        $user = auth()->user();
        $item = Content::where('id', $id)->where('user_id', $user->id)->first();
        
        $paid_features = VenuFeature::where('is_show', 1)->where('payment_status', 1)->get();
        $free_features = VenuFeature::where('is_show', 1)->where('payment_status', 0)->get();
        
        $sliders    = ContentSlider::where('content_id', $item->id)->where('is_show', 1)->get();
        
        $divisions = DB::table('location_divisions')->get();
        $districts = DB::table('location_districts')->get();
        $upazilas  = DB::table('location_upazillas')->get();
       
        return view(getTemplate() . '.user.content.venu_edit', ['sliders' => $sliders, 'free_features' => $free_features, 'paid_features' => $paid_features, 'item' => $item, 'user' => $user, 'divisions' => $divisions, 'districts' => $districts, 'upazilas' => $upazilas]);
    }
    
    public function venuUpdate($id, Request $request)
    {
        $sliders           = $request->file('slider_photos');
        $free_features     = $request->input('free_feature');
        $paid_features     = $request->input('paid_feature');
        $availability_days = $request->input('availability_day');
        $user = auth()->user();
        ...
        $newContent->upazila              = $request->input('upazila');
        $newContent->address              = $request->input('address');
        $newContent->free_extra_feature   = $this->getFreeFeature($free_features);
        $newContent->paid_extra_feature   = $this->getPaidFeature($paid_features);
        $newContent->venu_size            = $request->input('venu_size');
        $newContent->availability_type    = $request->input('availability_type');
        $newContent->availability_hour    = $request->input('availability_hour');
        $newContent->availability_day     = isset($availability_days) ? $this->getAvailibilityDay($availability_days) : null;
        $newContent->available_seat       = $request->input('available_seat');
        $newContent->discount_price       = $request->input('discount_price');
        $newContent->discount_price_type  = $request->input('discount_price_type');
        // $newContent->discount_price_days  = isset($price_in_days) ? json_encode($price_in_days) : null;
        ...
        return redirect('user/venu')->with('flash_success','Venu update successfully!');
    ...
    public function contentEdit($id)
    {
        ...
        $list = ContentCategory::withCount('contents', 'childs', 'filters')->where('parent_id', '0')->get();
        $instructor = InstructorInfo::where('id', $item->instructor_id)->first();
        $sliders    = ContentSlider::where('content_id', $item->id)->where('is_show', 1)->get();
        ...
        return view(getTemplate() . '.user.content.course_edit', ['instructors' => $instructors, 'sliders' => $sliders, 'facilities' => $facilities, 'menus' => $contentMenu, 'lists' => $list, 'item' => $item, 'instructor' => $instructor, 'divisions' => $divisions, 'districts' => $districts, 'upazilas' => $upazilas ]);
    }
    ...
    public function contentUpdate($id, Request $request)
    {
        ...
        $newContent->upazila              = $request->input('upazila');
        $newContent->address              = $request->input('address');
        $newContent->facilities           = $this->getFacility($facilities);
        $newContent->course_duration      = $request->input('course_duration');
        $newContent->course_duration_type = $request->input('course_duration_type');
        $newContent->course_start_date    = $request->input('course_start_time');
        $newContent->available_seat       = $request->input('available_seat');
        ...
        $newContent->save();
         ...
         return redirect('user/content')->with('flash_success','Course updated successfully!');
    ...
    protected function getFacility($facilityList){
        ...
        if(count($facilityList) > 0){
            ...
            $facilityArray = json_encode($facilities);
    ...
    protected function getFreeFeature($facilityList){
    ...
    protected function getPaidFeature($facilityList){
    ...
    protected function getAvailibilityDay($facilityList){
    ...
    public function contentPartUpdate(Request $request, $id)
    {
        ...
        if ($content) {
            $request->request->add(['mode' => 'request']);
            $update = ContentPart::find($id)->update($request->all());
            if($update)
                return 'ok';
            else
                return 'update error';
    ...
    public function contentMeetingItem($id)
    {
        ...
        $Content = Content::where('user_id', $user->id)->find($id);
        return view(getTemplate() .'.user.content.meeting.list', [
            'dates' => $dates,
            'meetings' => $meetings,
            'id' => $id,
            'content' => $Content
    ...
    public function userProfileView($id)
    {
        ...
        $courses = Content::where('user_id', $id)->where('type', '=', 'course')->orderBy('id', 'DESC')->paginate(16);
        $venus   = Content::where('user_id', $id)->where('type', '=', 'venu')->orderBy('id', 'DESC')->paginate(16);
        $galleries   = Gallery::where('user_id', $id)->where('is_show', '=', 1)->orderBy('id', 'DESC')->paginate(16);
        $instructors = UserInstructor::where('user_id', $id)->where('is_show', '=', 1)->orderBy('id', 'DESC')->paginate(8);
        ...
        $data['footer_ad']          = AdsBox::where('mode', 'publish')->where('position', '=', 'profilepage-before-footer')->orderBy('id', 'DESC')->first();
        $data['sidebar_top_ad']     = AdsBox::where('mode', 'publish')->where('position', '=', 'profilepage-right-side-top')->orderBy('id', 'DESC')->first();
        $data['sidebar_bottom_ad']  = AdsBox::where('mode', 'publish')->where('position', '=', 'profilepage-right-side-bottom')->orderBy('id', 'DESC')->first();
        ...
        try{
            ...
            $Checkout = Session::create([
                ...
                'line_items' => [[
                    'price_data' => [
                        'currency' => currency(),
                        'unit_amount' => currency() == $Transaction->price*100,
                        'product_data' => [
                            'name' => $content->title,
                        ],
                    ],
    ...
    public function QuizzesQuestionsStore(Request $request, $quiz_id)
    {
        ...
        if (!empty($quiz)) {
            ...
            $question_data = [
                ...
                'type' => $data['type'],
                'created_at' => time(),
                'answer_video'=>isset($data['answer_video'])?$data['answer_video']:null
            ];
    ...
    public function QuizzesReview($quiz_id, Request $request){
        ...
        if ($quiz) {
            ...
            $data = [
                'quiz'      => $quiz,
                'answers'   => isset($answers->result)?json_decode($answers->results,true):''
            ];
    ...
    public function videoLiveList()
    {
        ...
        return view(getTemplate() .'.user.meeting.list', [
            'courses' => $courses,
            'list' => $list
        ]);
    }
    ...
    public function videoLiveUsers($id)
    {
        ...
        return view(getTemplate() .'.user.meeting.users', ['list' => $list->paginate(30)]);
    }
