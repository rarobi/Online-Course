...
class ApiController extends Controller
{
    ...
    public function functionIndex(Request $request){
        ...
        if(get_option('gateway_razorpay',0) == 1) $data['gateway'][] = 'razorpay';
        ## Get Blog Posts
    ...
    public function productVerify(Request $request){
        ...
        if($request->gateway == 'razorpay'){
            ...
            if($order == null){
                ...
                return \redirect('/')->with('msg', trans('admin.payment_failed'));
            }
        }
        if($request->gateway == 'credit'){
            return view(getTemplate() . '.app.verify');
    ...
    public function productCaptivate($id, $pid,Request $request){
        $User = $this->checkUserToken($request);
        if(!$User)
            return 'no user';
