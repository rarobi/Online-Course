    ...
    public function handle($request, Closure $next)
    {
        ...
        if (auth()->check()) {
            ...
            if ($admin->isAdmin()) {
                ...
                if (!empty($capatibilty) and $capatibilty != 'all' and !in_array(Request::segment(2), $capatibilty) and Request::segment(2) != 'profile' and Request::segment(2) != 'video' and Request::segment(2) != 'about' and $admin->username != 'admin')
                    return abort(404);
                global $Access;
                $Access = $capatibilty;
