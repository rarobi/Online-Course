...
class NotificationMiddleware
{
    public function handle($request, Closure $next)
    {
        ...
        if (auth()->check()) {
            ...
            $notification =  Notification::where('recipent_list', $user->id)->get();




            foreach ($notification as $noti) {
                if (!checkNotificationStatus($noti->id, $user->id))
                    $alert['notification']++;
            }
            ...
            $Sell_download =  Sell::where('user_id', $user->id)->where('view', 0)
                    ->where('type', 'download')
                    ->get();

            $alert['sell_download'] = count($Sell_download);

            $Sell_post = Sell::where('user_id', $user->id)
                    ->where('view', 0)
                    ->where('type', 'post')
                    ->get();
